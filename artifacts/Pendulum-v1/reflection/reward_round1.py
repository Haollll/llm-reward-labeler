def reward(obs, action, next_obs):
    ...
    stability_penalty = -0.05 * np.clip(theta_dot ** 2, 0, None)  # Penalty for angular velocity (stability)
    action_cost = -0.001 * np.clip(torque ** 2, 0, None)  # Penalty for torque usage (energy cost)
